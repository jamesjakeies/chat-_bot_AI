...
import com.xinyu.ai.domain.usecase.RegisterUseCase
...
class RegisterViewModel @Inject constructor(
    ...
    fun register(email: String, password: String, nickname: String) {
        ...
        viewModelScope.launch {
            ...
            when (val result = registerUseCase(email.trim(), password, nickname.trim())) {
                is AppResult.Success -> {
                    refreshCurrentUserUseCase()
                    observeCurrentUserUseCase().first()
                    _uiState.value = UiState.Success(Unit)
                    _navigationTarget.emit(AppDestination.AgeGate.route)
                }
